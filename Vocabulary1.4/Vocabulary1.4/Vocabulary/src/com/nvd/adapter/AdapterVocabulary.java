...
public class AdapterVocabulary extends ArrayAdapter<vocabulary>{
	...
	public AdapterVocabulary(Context c, int resource,
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		//
		Typeface facev=Typeface.createFromAsset(context.getAssets(),"fonts/TIMESBD.TTF"); // tạo kiểu chữ tiếng việt 
		Typeface facee=Typeface.createFromAsset(context.getAssets(),"fonts/TIMESI.TTF"); // tạo kiểu chữ tiếng anh 
		//
		vocabulary v = data.get(position);
		// anh xa
		TextView txtWordEnglish = (TextView)view.findViewById(R.id.word_english);
		TextView txtWordVietNam = (TextView)view.findViewById(R.id.word_VietNam);
		ImageView icon_music = (ImageView)view.findViewById(R.id.icon_volue);
		ImageView icon_mic = (ImageView)view.findViewById(R.id.icon_mic);
		final ImageView icon_yeuthich_true  = (ImageView)view.findViewById(R.id.icon_yeuthich_true);
		final ImageView icon_yeuthich_false = (ImageView)view.findViewById(R.id.icon_yeuthich_false);
		//
		txtWordVietNam.setTypeface(facev); // 
		txtWordVietNam.setTypeface(facee); //
		MediaPlayer media = new MediaPlayer();
		
		txtWordEnglish.setText(v.getWordEnglish());
		txtWordVietNam.setText(v.getWordVietNam());
		media.setAudioSessionId(v.getAudio());
		if(v.getYEUTHICH()==1){
		...
		// bắt sự kiện 
					icon_music.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							...
							mp = MediaPlayer.create(context, data.get(i).getAudio());
							mp.start();
